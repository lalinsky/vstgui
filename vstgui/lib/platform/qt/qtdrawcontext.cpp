...
void QtDrawContext::drawBitmap (CBitmap* bitmap, const CRect& dest, const CPoint& offset, float alpha)
{
	...
	const auto image = ( static_cast<QtBitmap*> (bitmap->getPlatformBitmap ()) )->getQImage ();
	const auto source = QRectF (offset.x, offset.y,	image.width () - offset.x, image.height () - offset.y);
